...
from treform.utility.base_util import is_my_pc
from treform.utility.file_util import FileUtil
...
class LogUtil(object):
    ...
    def __log_path_from_path(source_file, sub_log_dir='logs'):
        ...
        """
        _dir = os.path.join(os.path.dirname(str(source_file)), str(sub_log_dir))
        _basename = os.path.basename(str(source_file))
        if len(sys.argv) > 1:
    ...
    def __create_logger(log_path, error_log_path=None, level=logging.NOTSET, console_mode=False, format=format,
        ...
        if log_path:
            if multiprocess:
                from .mult_proc_timed_rotating_file_handler import MultProcTimedRotatingFileHandler
